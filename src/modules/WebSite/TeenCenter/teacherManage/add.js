  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        dispatch(actions.addTeacher({ primeTeacher: reqBean }))
      }
    })
  }

  render() {
    const { token, classAll, showButtonSpin } = this.props
    const { getFieldDecorator } = this.props.form
    const { previewImage, previewVisible, teacherImage } = this.state
    return (
      <div>
