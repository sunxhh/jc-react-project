...
class EditStudent extends Component {
  ...
  componentWillMount() {
    ...
    dispatch(saleName())
    dispatch(getStudentDetail({ id: id })).then(res => {
      if (res) {
        this.setState({
          courseCoverImg: res.image ? [{
            url: res.image,
            uid: -1
          }] : []
        })
    ...
    dispatch(getCourseList({ course: {}, currentPage: 1, ...this.props.coursePagination }))
  }

  componentWillReceiveProps(nextProps) {
    if (this.props.studentDetail !== nextProps.studentDetail) {
      const course = nextProps.studentDetail.course
      this.setState({ courseIds: course ? course.courseIds : '' })
    }
  }
  ...
  handleChangeBanner = (file) => {
    ...
    if (isEmpty(file.fileList) || (isImageType && isImageSize)) {
      this.setState({
  ...
  handleSubmit = () => {
    this.props.form.validateFields((err, values) => {
      if (!err) {
        this.props.dispatch(editStudentList({
          student: {
            name: values.name,
            ...
            image: values.image && values.image.status !== 'removed' ? values.image.url : '',
            id: this.props.match.params.studentId,
          },
          courseIds: this.state.courseIds ? this.state.courseIds.split(',') : []
        }))
      }
    })
  }
  ...
  render() {
    ...
    const { studentDetail, channelListAll, queryAllUserList, getStudentLinkType, getStudentType, getStudentlevel, getCourseType } = this.props
    const { selectedRowKeys, previewVisible, previewImage } = this.state
    ...
    const channelIdInitValue = studentDetail.channelId && channelListAll.filter(item => item.id === studentDetail.channelId).length > 0
      ? { initialValue: studentDetail.channelId }
      : {}
                    ...
                    <Col span={7}>
                      ...
                      >
                        {getFieldDecorator('name', {
                          initialValue: studentDetail.name ? studentDetail.name : '',
                          rules: [{
                            required: true,
                            message: '请输入姓名',
                          }],
                    ...
                    <Col span={7}>
                      ...
                      >
                        {getFieldDecorator('linkPhone', {
                          initialValue: studentDetail.linkPhone ? studentDetail.linkPhone : '',
                          rules: [{
                            required: true,
                            message: '请填写联系电话',
                          }],
                    ...
                    >
                      ...
                      >
                        {getFieldDecorator('linkType', {
                          initialValue: studentDetail.linkType ? studentDetail.linkType : '',
                          rules: [{
                            message: '请选择所属关系',
                          }],
                        })(
                    ...
                    >
                      ...
                      >
                        {getFieldDecorator('sex', {
                          initialValue: studentDetail.sex ? studentDetail.sex : '',
                          rules: [{
                            required: true,
                            message: '请选择性别',
                          }],
                    ...
                    >
                      ...
                      >
                        {getFieldDecorator('studentType', {
                          initialValue: studentDetail.studentType ? studentDetail.studentType : '',
                          rules: [{
                            required: true,
                            message: '请选择学员类型',
                          }],
                    ...
                    >
                      ...
                      >
                        {getFieldDecorator('intentLevel', {
                          initialValue: studentDetail.intentLevel ? studentDetail.intentLevel : '',
                        })(
                          <Select
                    ...
                    >
                      ...
                      >
                        {getFieldDecorator('salerId', {
                          initialValue: studentDetail.salerId ? studentDetail.salerId : '',
                        })(
                          <Select
                            placeholder='请选择销售人员'
                            showSearch={true}
...
export default connect(mapStateToProps, mapDispatchToProps)(Form.create()(EditStudent))
