...
class Edit extends Component {
  ...
  componentWillMount() {
    ...
    dispatch(actions.getActivityDetail({ id: id })).then(res => {
      const newState = {}
      if (!isEmpty(res.image)) {
        newState.activityImage = [{ uid: '1', url: res.image }]
      }
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.activityDetail !== nextProps.activityDetail) {
      this.setState({
        editorDraft: nextProps.activityDetail.intruduction,
        actEndTime: nextProps.activityDetail.endTime
      })
    }
  }
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        dispatch(actions.editActivity(reqBean))
      }
    })
  }
  ...
  _disabledEnrollDate = (current) => {
    let date = new Date().getFullYear() + '-' + (new Date().getMonth() + 1) + '-' + new Date().getDate()
    let currDate = moment(current).format('YYYY-M-DD')
    return current && (new Date(currDate).valueOf() < new Date(date).valueOf() || current.valueOf() > moment(new Date(moment(this.state.actEndTime).format('YYYY-MM-DD')).valueOf() + (1000 * 60 * 60 * 24)).valueOf())
  }

  render() {
    const { token, activityDetail } = this.props
    const { getFieldDecorator } = this.props.form
    const { previewImage, previewVisible, activityImage, isJumpStatus, isEnrollStatus, editorDraft } = this.state
    return (
      <div>
            ...
            <Col span={16}>
              ...
              >
                {getFieldDecorator('name', {
                  ...
                  initialValue: activityDetail.name
                })(
                  <Input
                    placeholder='请输入活动名称'
                  />
              ...
              >
                {getFieldDecorator('sort', {
                  ...
                  initialValue: activityDetail.sort
                })(
                  <Input
                    placeholder='请输入排序'
                  />
              ...
              >
                ...
                >
                  {getFieldDecorator('activityTime', {
                    ...
                    initialValue: [
                      activityDetail.startTime ? moment(activityDetail.startTime) : '',
                      activityDetail.endTime ? moment(activityDetail.endTime) : ''
                    ],
                  })(
                    <RangePicker
              ...
              >
                {getFieldDecorator('image', {
                  ...
                  initialValue: this.imageUpload({ fileList: activityImage }),
                })(
                  <OrderUpload
              ...
              >
                {getFieldDecorator('forwardFlag', {
                  initialValue: activityDetail.forwardFlag,
                  rules: [{
                    required: true,
                    message: '请选择是否跳转'
                  }],
                ...
                isJumpStatus === '1' && (
                  ...
                  >
                    {getFieldDecorator('forwardUrl', {
                      ...
                      initialValue: activityDetail.forwardUrl,
                    })(
                      <Input
                        placeholder='请输入URL'
                      />
                ...
                isJumpStatus === '0' && (<div>
                  ...
                  >
                    {getFieldDecorator('needEnrollFlag', {
                      initialValue: activityDetail.needEnrollFlag,
                      rules: [{
                        required: true,
                        message: '请选择是否需要报名'
                      }]
                      ...
                      >
                        ...
                        >
                          {getFieldDecorator('enrollTime', {
                            ...
                            initialValue: [
                              activityDetail.enrollStartTime ? moment(activityDetail.enrollStartTime) : '',
                              activityDetail.enrollEndTime ? moment(activityDetail.enrollEndTime) : ''
                            ],
                          })(
                            <RangePicker
                              ...
                              style={{ width: '100%' }}
                              format='YYYY-MM-DD HH:mm:ss'
                              disabledDate={current => this._disabledEnrollDate(current)}
                              disabledTime={disabledRangeTime}
                              showTime={{ hideDisabledOptions: true }}
                            />)}
                        </div>
