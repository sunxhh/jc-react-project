...
export function flatten(tree, stocks = [], options) {
  let { optionValue = 'id', optionText = 'text', childOptionValue = 'id', childOptionText = 'text', leafOptionValue = 'leaf' } = options || {}
  let result = []
  let skuLen = 0
  ...
  tree.forEach(sku => {
    const leaf = sku[leafOptionValue]
  ...
  for (let i = 0; i < skuLen; i++) {
    ...
    tree.forEach((sku, column) => {
      ...
      if (leaf.length > 1) {
        ...
        item = tree[column][leafOptionValue][row]
      } else {
        item = tree[column][leafOptionValue][0]
      }
      if (!sku[optionValue] || !item[childOptionValue]) return
      mapKey.push(`${sku[optionValue]}_${item[childOptionValue]}`)
      skus.push({
        k_id: sku[optionValue],
        k: sku[optionText],
        v_id: item[childOptionValue],
        v: item[childOptionText],
      })
    })
